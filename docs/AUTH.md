...
Projects, pipelines, tasks, models, artifacts, memory, and knowledge vectors are scoped to an **owner id**:
...
- Schema: `owner_id TEXT NOT NULL DEFAULT 'local'` on `projects`, `pipelines`, `tasks`, `models`, and `artifacts`. Existing SQLite/Postgres DBs get an additive `ALTER TABLE … ADD COLUMN` on store init (`SqlDb.ensure_column`).
- **Memory** is partitioned by `owner_id` in `MemoryStore` (legacy flat `memory.json` migrates into the `"local"` bag).
- **Qdrant** points include `owner_id` in the payload and point id; searches apply an `owner_id` filter.
